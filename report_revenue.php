...
if($tahun == null || $tahun == "" ){
        ...
        $sqla="SELECT no, COUNT( * ) AS totaldata FROM $forward where nota IN (SELECT nota FROM transaksimasuk) and tglmasuk LIKE '$tahun-$bulan-%'";
      }
...
?>
  ...
  <?php
$sqlb="SELECT SUM(total) AS total FROM $forward WHERE nota IN (SELECT nota FROM transaksimasuk) AND tglmasuk LIKE '$tahun-$bulan-%'";
...
<?php if($tahun != null || $tahun != ""){ ?>
 ...
 ?>
     ...
     if ($tahun != null || $tahun != "") {
         ...
         $query1="SELECT * FROM  $forward where nota IN (SELECT nota FROM transaksimasuk) and tglmasuk like '$tahun-$bulan-%' order by no limit $rpp";
